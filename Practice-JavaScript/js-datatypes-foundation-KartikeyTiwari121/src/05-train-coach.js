...
export function findPassenger(passengers, name) {
  if(Array.isArray(passengers)===false || typeof name !== "string" ){
   return undefined;
  }
  
  return passengers.find(passengerCurrent => passengerCurrent.name.toLowerCase() === name.toLowerCase());
}
...
export function getPassengerIndex(passengers, name){
  ...
  return passengers.findIndex(passengerCurrent => passengerCurrent.name.toLowerCase() === name.toLowerCase());
}
...
export function isAnyWaitlisted(passengers) {
  ...
  return passengers.some(passengerCurrent => passengerCurrent.status.toLowerCase() === "waitlisted");
}
...
export function areAllConfirmed(passengers) {
  ...
  return passengers.every(passengerCurrent => passengerCurrent.status.toLowerCase() === "confirmed");
}

export function getWaitlistedPassengers(passengers) {
  if(Array.isArray(passengers)===false){
   return [];
  }
  return passengers.filter(passengerCurrent => passengerCurrent.status.toLowerCase() === "waitlisted");
}
