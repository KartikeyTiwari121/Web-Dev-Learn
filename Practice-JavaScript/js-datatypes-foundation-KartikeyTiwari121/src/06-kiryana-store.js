...
export function getAffordableItems(items, maxPrice) {
  if(Array.isArray(items)===false || Number.isNaN(maxPrice) || typeof maxPrice !== "number"){
   return [];
  }
...
export function formatBill(items) {
  if(Array.isArray(items)===false || items === ""){
   return "";
  }
  let updatedItems = items.map(item => `${item.name} x ${item.qty} = Rs.${item.price*item.qty}`);
