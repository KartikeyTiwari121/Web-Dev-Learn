...
export function addUrgentItem(cart, item) {
  ...
  cart.unshift(item);
  //direct cart.unshift(item) return me nhi dala kynki ye cart.unshift(item) direct updated array ki length dega na ki array
  return cart;
}
...
export function mergeCarts(cart1, cart2) {
  ...
  else if(Array.isArray(cart2)===false){
    cart2 = [];
  }
