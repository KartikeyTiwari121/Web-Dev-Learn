...
export function createThaliDescription(thali) {
 if(thali===null || typeof thali !== "object" || typeof thali.name !== "string" || !Array.isArray(thali.items) || typeof thali.price !== "number" || typeof thali.isVeg !== "boolean"){
  return "";
 }
...
export function generateThaliReceipt(customerName, thalis) {
  ...
  let cName = customerName.toUpperCase();
  let tPrice = thalis.reduce((sum, thali)=>{
    return sum+thali.price;
  },0);
  let itemCount = thalis.length;

  let lineItem = thalis.map(thali =>{
