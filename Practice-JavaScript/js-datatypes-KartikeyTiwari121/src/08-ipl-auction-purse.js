...
export function iplAuctionSummary(team, players){
  if(typeof team !== "object" || team === null || typeof team.purse !== "number" || team.purse <= 0 || !Array.isArray(players) || players.length === 0){
    return null;
  }
  let teamName = team.name;
  let playerCount = players.length;
  let totalSpent = players.reduce((sum, playerObj)=>{
    return sum + playerObj.price;
  },0);
  ...
  let costliestPlayer = sortedPlayers[sortedPlayers.length -1];
  let cheapestPlayer = sortedPlayers[0];
  let averagePrice = Math.round(totalSpent/playerCount);

  let byRole = {};
  for (const player of players) {
  if (!byRole[player.role]) {
    byRole[player.role] = 0;
  }
  byRole[player.role]++;
  }

  return {
    teamName, totalSpent, remaining, playerCount, costliestPlayer, cheapestPlayer, averagePrice, byRole, isOverBudget
  };
}
