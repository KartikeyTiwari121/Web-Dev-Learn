...
export function maskAadhaar(aadhaarNumber) {
  if(typeof aadhaarNumber !== "string" || aadhaarNumber.length !== 12 || aadhaarNumber.includes(" ")|| aadhaarNumber.includes("-")){
    return "INVALID";
  }
  ...
  for(let i=0; i< aadhaarNumber.length; i += 4){
    if(i < 8){
      resultArr.push("X".repeat(4));
    }
    else{
    resultArr.push(aadhaarNumber.slice(i, i+4));
    }
  }
